...
export class EventService {
  ...
  getEvents(): Observable<IEvent[]>{
    // let subject = new Subject<IEvent[]>(); //mock service
    // setTimeout(() => {
    //   subject.next(events);
    //   subject.complete();
    // }, 100);
  ...
  getEvent(id: number) : Observable<IEvent> {
    //let event: IEvent = events.find(e=>e.id === id)!; //non-null assertion - zapewnienie TS ze jestesmy pewni, ze obj nie bedzie null
